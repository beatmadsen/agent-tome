...
class WorkflowConsolidateFetchTest < Minitest::Test
  ...
  def test_consolidate_fetch_lifecycle
    ...
    assert addend1.success?, "Addend 1 failed: #{addend1.error_message}"

    addend2 = tome.addend(original_id, body: "Incremental GC added in Ruby 2.2.")
    assert addend2.success?, "Addend 2 failed: #{addend2.error_message}"

    addend3 = tome.addend(original_id, body: "GC.compact introduced in Ruby 2.7.")
    assert addend3.success?, "Addend 3 failed: #{addend3.error_message}"
    ...
    assert_match(/\A[1-9A-HJ-NP-Za-km-z]{7}\z/, old_article_id)

    # Step 3: Fetch via original ID — returns the consolidated article with consolidated_from
    fetch_new = tome.fetch(original_id)
    assert fetch_new.success?, "Fetch new failed: #{fetch_new.error_message}"
    ...
    assert fetch_old.success?, "Fetch old failed: #{fetch_old.error_message}"
